...
def test_add_comment(client, headers, payload):
    ...
    comment_count = client.get(f"/posts/{post_id}", headers=headers).json()[
        "comment_count"
    ]
...
class TestGetCommentsBaseCommentsWithOneLevelReply(Base):
...
class TestGetCommentsBaseCommentsWithTwoLevelReply(Base):
    @pytest.mark.parametrize(
        ("reply_level", "comments_count"),
        [("0", 5), ("1", 10), ("2", 15), ("3", 15)],
    )
...
class TestGetCommentsBaseCommentsWithThreeLevelReply(Base):
    @pytest.mark.parametrize(
        ("reply_level", "comments_count"),
        [("0", 5), ("1", 10), ("2", 15), ("3", 20)],
    )
